...
class EmailService {
    ...
    static async createTemplate(templateName, subject, email, templateHTML, templateData) {
        const params = {
            'Template': {
                'TemplateName': templateName, // This is the template name which you'll refer while sending through SES.
                'SubjectPart': subject,
                'HtmlPart': templateHTML
            }
        };
        ses.getTemplate({ TemplateName: templateName }, async (err, data) => {
            if (err) {
                try {
    ...
    static async sendEmail(templateName, email, templateData) {
        ...
        try {
            ...
            Logger.error(`sendEmail: + ${templateName}`, error);
        }
    }
}
